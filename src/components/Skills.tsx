      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredSkills.map((skill, index) => (
            <div
              key={index}
              className="bg-[#222831] p-6 rounded-xl hover:bg-[#222831]/80 transition-all duration-300 hover:scale-105 hover:shadow-lg border border-[#00ADB5]/10 hover:border-[#00ADB5]/30"
            >
              ...
              <div className="flex items-center justify-between">
                <span className="text-[#EEEEEE]/60 text-sm">{skill.category}</span>
                <span className="text-[#00ADB5] text-sm font-medium">{skill.level}</span>
              </div>
            </div>
          ))}
        </div>
